...
FragTrap::FragTrap(std::string const &name): _name(name), _hitPoints(MAX_F_HIT_P), _energyPoints(MAX_F_ENERGY_P), _level(1), _meleeDamage(30), _rangeDamage(20), _armorDamageReduction(5) {
...
void FragTrap::rangedAttack(std::string const & target) {
    std::cout << "FR4G-TP (FragTrap) <" << this->_name << "> attacks <" << target << "> at range, causing <" << _rangeDamage << "> points of damage!" << std::endl;
    std::cout << this->_name << " <Is that what people look like inside?>" << std::endl;
};

void FragTrap::meleeAttack(std::string const & target) {
    std::cout << "FR4G-TP (FragTrap) <" << this->_name << "> attacks <" << target << "> by melee attack, causing <" << _meleeDamage << "> points of damage!" << std::endl;
...
void FragTrap::takeDamage(unsigned int amount) {
    ...
    if (this->_hitPoints - dmg < 0) {
		std::cout << "FR4G-TP (FragTrap) <" << this->_name << "> takes <" << this->_hitPoints << "> points of damage!" << std::endl;
        std::cout << this->_name << " <If only my chassis... weren't made of recycled human body parts! Wahahaha!>" << std::endl;
        this->_hitPoints = 0;
    }
    else {
		std::cout << "FR4G-TP (FragTrap) <" << this->_name << "> takes <" << dmg << "> points of damage!" << std::endl;
...
void FragTrap::beRepaired(unsigned int amount) {
    if (this->_hitPoints + amount > this->_maxHitPoints) {
		std::cout << "FR4G-TP (FragTrap) <" << this->_name << "> repairs <" << this->_maxHitPoints - this->_hitPoints << "> hit points!" << std::endl;
        std::cout << this->_name << " <I'm so sexy!!!>" << std::endl;
        this->_hitPoints = _maxHitPoints;
    }
    else {
		std::cout << "FR4G-TP (FragTrap) <" << this->_name << "> repairs <" << amount << "> hit points!" << std::endl;
...
int FragTrap::vaulthunter_dot_exe(std::string const & target) {
	...
	if (energy < 0) {
		...
		std::cout << "FR4G-TP (FragTrap) <" << this->_name << "> is out of energy" << std::endl;
		std::cout << this->_name << " <I'm too depressed to do this, let me die quietly!!!>" << std::endl;
	}
	else {
		this->_energyPoints = energy;
		std::cout << "FR4G-TP (FragTrap) <" << this->_name << "> attacks " << target << " with <" << list[attack] << ">, causing <"
