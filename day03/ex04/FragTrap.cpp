...
int FragTrap::vaulthunter_dot_exe(std::string const & target) {
	...
	if (energy < 0) {
		...
		std::cout << GREEN << this->_name << CYAN << " <I'm too depressed to do this, let me die quietly!!!>"
		<< RESET << std::endl;
	}
	else {
		this->_energyPoints = energy;
		std::cout << CYAN << "FR4G-TP <" << MAGENTA << this->_name << CYAN << "> attacks " << MAGENTA << target
					<< CYAN << " with <" << MAGENTA << list[attack] << ">, causing <" << MAGENTA
					<< dmg[attack] << CYAN << "> points of damage!" << RESET << std::endl;
