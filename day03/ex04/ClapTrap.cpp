...
ClapTrap::ClapTrap(): _name("Clappy"), _hitPoints(10), _maxHitPoints(10), _energyPoints(10),
					  _maxEnergyPoints(10), _level(10), _meleeDamage(10), _rangeDamage(10), _armorDamageReduction(10) {
	std::cout << "Default constructor called" << std::endl;
	std::cout << GREEN << this->_name << CYAN << ": <Oh no, it's a Trap, ClapTrap!!!>" << RESET << std::endl;
}

ClapTrap::ClapTrap(std::string const &name): _name(name), _hitPoints(10), _maxHitPoints(10), _energyPoints(10),
					_maxEnergyPoints(10), _level(10), _meleeDamage(10), _rangeDamage(10), _armorDamageReduction(10) {
    std::cout << "Name constructor called" << std::endl;
    std::cout << GREEN << this->_name << CYAN << ": <Oh no, it's a Trap, ClapTrap!!!>" << RESET << std::endl;
}

ClapTrap::ClapTrap(std::string const &name, unsigned int const &hitP, unsigned int const &maxHit,
				   unsigned int const &energy, unsigned int const &maxEnergy, unsigned int const &level,
				   unsigned int const &meleeDmg, unsigned int const &rangeDmg, unsigned int const &armor):
				   _name(name), _hitPoints(hitP), _maxHitPoints(maxHit), _energyPoints(energy),
				   _maxEnergyPoints(maxEnergy), _level(level), _meleeDamage(meleeDmg), _rangeDamage(rangeDmg),
